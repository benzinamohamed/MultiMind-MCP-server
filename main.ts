...
const server = new Server({
  ...
  const dialogue = async (dialoguePhase : LLMtought)=>{
    ...
    try {
      ...
      critique = "Error calling the critic LLMs or critics not available, check the tool configuration or the critic LLMs availability";
      throw new Error(`Error calling the critic LLMs or critics not available, check the tool configuration or the critic LLMs availability: ${error instanceof Error ? error.message : String(error)}`);
    }
    return {
      tought: tought,
...
server.setRequestHandler(ListToolsRequestSchema , async () => ({
  ...
  - critique: A structured response from the critic LLM highlighting flaws, assumptions , gaps, or missteps ,keep this empty no matter what ,dont fill it yourself , this will be filled by the critic LLM automatically in next step , if theres no critique generated by the critic LLM or an error occurs fill this with "No critique generated or an error occurred" 
  ...
  3. Receive Critique: The selected critic LLM reviews the thought and issues a structured critique — focusing on logic, assumptions, facts, or alternatives , if the critic LLMs are not available or an error occurs in the critic LLMs sets LoopContinue = false , and tell the user what happened, "
 ...
 - (important!) if the critics aren't available or an error occurs in the critic LLMs , return "Error calling the critic LLMs or critics not available , check the tool configuration or the critic LLMs availability" 
  ...
  async (request) => {
    if (request.params.name === "MultiMind") {
      try {
        ...
        throw new Error(`Error processing dialogue: ${errorMessage}`);
      }
    }
